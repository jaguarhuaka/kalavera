  ...
  try {
    ...
    if (tx_id) {
      ...
      if (data.error || !data.result?.transfer) {
        return new Response(JSON.stringify({ status: "not_found", message: "TX no encontrada" }), { status: 200, headers });
      }
    ...
    if (address_index === undefined || !amount_atomic) {
      return new Response(JSON.stringify({ error: "Se requiere address_index + amount_atomic, o tx_id" }), { status: 400, headers });
    }
    ...
    if (!validTx) {
      return new Response(JSON.stringify({
        status: "waiting", confirmations: 0, required: REQUIRED_CONFIRMATIONS, mode: "auto"
      }), { status: 200, headers });
    }
